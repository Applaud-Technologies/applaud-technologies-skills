...
When the user wants to add an integration, first identify which type they need, then read the specific integration guide from the `AdapterIntegrations/` folder.
...
Once the user selects an integration type, read the corresponding guide file from the `AdapterIntegrations/` folder and follow its instructions.
